...
Create a new file titled `Example.js` in the `src/` directory, and add the code below which uses the `Worldview` component to render a red cube and x/y/z axes:
...
Most of the existing [Commands](#/docs/api/command) support instancing, which is why they accept **arrays** of objects as children. In this step, let's convert our previous example to use instanced rendering.
