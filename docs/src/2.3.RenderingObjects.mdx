...
Create a new file titled `Example.js` in the `src/` directory, and add the code below to render a Worldview component with a red Cube and x/y/z Axes:
...
You can interact with Worldview via touch-screen to move (using 1 finger) and rotate the scene (using 2 fingers). Learn more about [controlling the camera with your keyboard](#/docs/api/worldview).
...
Whenever you click the "Add a Cube” button, you'll see a new Cube with a randomly generated color getting added to the scene.
...
// src/Example.js
function Example() {
  const steps = 500; // total # of objects
...
In our new example, we are rendering a single marker with 500 colors and points, instead of 500 separate markers. This change, while change, will save us a significant amount of CPU -> GPU communication time!

<CodeSandboxEmbed src="https://codesandbox.io/s/jl9qv2mjyy" />

Next: [Managing the Camera](#/docs/tutorial/managing-camera)
