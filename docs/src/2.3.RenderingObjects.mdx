...
Create a new `Example.js` file in the `src/` directory, add below code which create a Worldview component with a red cube and x/y/z axes inside:
...
export default function Example() {
  ...
  const markers = new Array(count).fill().map((_, idx) => {
    return {
      ...
      // the rgba value of the color is between 0 and 1 (inclusive).
...
function Example() {
  ...
  // the object index needs to multipled by this scale so it's evenly distributed in the space
