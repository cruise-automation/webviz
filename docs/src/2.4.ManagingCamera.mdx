...
For continuously moving the camera around, we' use need to create a state that get's updated overtime. [window.requestAnimationFrame](https://developer.mozilla.org/en-US/docs/Web/API/window/requestAnimationFrame) is a good direction since it generally matches the display refresh rate. we'll add a [React Hook](https://reactjs.org/docs/hooks-intro.html) that's going accept a callback which is going to be called before each browser repaint.
...
_Notice we've added `cameraState={cameraState}` which turned worldview into a [controlled](https://reactjs.org/docs/forms.html) component which usually requires an `onChange` handler. In our case here, since we don't need to know what's the new internal
...
In this step, we are going to move the duck along the knot, and move the camera as if we are following the duck around.
