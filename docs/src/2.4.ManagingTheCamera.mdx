...
The WebGL [camera](https://webglfundamentals.org/webgl/lessons/webgl-3d-camera.html) is designed to mimic the way a human eye sees. We've developed a simple and intuitive API that abstracts away much of the math needed to control the camera via `cameraState`. Read the [Camera docs](#/docs/api/camera) to use the playground and learn more.

In this step, let's add a duck model to the center of the scene, and move the camera to observe the duck from different angles.

First, let's create a Duck component using [GLTFScene](#/docs/commands/gltfscene), which we'll use to compose with other Command components, and add the `Duck.glb` model file to the `src/utils` folder.
...
To move the camera, we will need to create a state that gets updated over time. [window.requestAnimationFrame](https://developer.mozilla.org/en-US/docs/Web/API/window/requestAnimationFrame) generally matches the display refresh rate. Let's add a [React Hook](https://reactjs.org/docs/hooks-intro.html) that accepts a callback which will be called before each browser repaint.
...
Lastly, let's hook everything together. We'll use `useRequestAnimationFrame` to increase a counter by `1` in every requestAnimaitonFrame callback, and use the counter to adjust the camera's `phi` and `thetaOffset` accordingly.
...
Next, let's remove the camera setting for `thetaOffset` and `phi`, so that we don't feel dizzy observing the scene while the duck is moving. We can derive the duck position from the spheres over time, and use the duck position as `target` input for `cameraState`.
...
Voila! By adding the `target` position to `cameraState`, the camera now folllows the duck around!

<CodeSandboxEmbed src="https://codesandbox.io/s/71k2xzj8ox" />

Next: [Adding Interactivity](#/docs/tutorial/adding-interactivity)
