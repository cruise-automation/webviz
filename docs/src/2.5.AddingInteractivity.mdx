...
In this section, we are going to place obstacles (red cubes) along the duck's moving path, and when the duck hit the obstacles, it's going to stop moving until you removed the corresponding obstacles.

First we are going to add `id` field to the sphere marker to enable hitmap so we scan correctly identify the specific sphere that's being clicked.
...
const obstacleMarkers = Array.from(clickedObjectIds).map((clickedObjectId, index) => {
  ...
  return {
    ...
    scale: { x: 4, y: 4, z: 4 }, // bigger than the spheres
  };
});
```
...
```js
 <Spheres
    onClick={(ev, { objectId, object }) => {
        ...
        // hitmap, clicking again will only trigger `onClick` event on the Cube because it's a lot bigger
...
Put it all together, we have a moving duck that get stuck by every red cube obstacles we put on it's path. Click the cube that the duck is stuck to remove it and the duck will continue moving forward.
...
- Read the [API](#/docs/api) docs to learn about some of the common APIs for Worldview, camera, mouse interaction, and type support.
- Learn about all the [Commands](#/docs/commands) such as Arrows, Cubes, Spheres, Triangles, and Lines that we support.
