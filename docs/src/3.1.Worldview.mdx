...
| `hitmapOnMouseMove`   | `boolean`                                       |                        | if enabled, the clicked object will be returned from the mouse event handler. Disabled by default for performance reasons. |
| `disableHitmapForEvents`   | `[MouseEventEnum]`                         |                        | disable rendering the hitmap for some mouse events for performance reasons. Only "onMouseMove" is disabled by default. |
