...
You can move the camera around using the mouse/trackpad, or by using the keyboard: _a/s/d/f_ (for `targetOffset`), _q/e_ (for `thetaOffset`), _r/f_ (for `phi`), and _z/x_ (for `distance`).
