...
| `mapObjectToInstanceCount`  | yes      | `(T) => number`                                                       |         | map the marker object to the number of instances it has. See "Instanced Commands" below             |
...
// example for supporting interactions, for details check the MouseEvents documentation
const Cubes = makeCommand("Cubes", rawReglCommand, {
