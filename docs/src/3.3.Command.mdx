...
| `getHitmapId`         | yes      | `(T, pointIndex?: number) => ?number`                       |         | map the marker object to a hitmapId                                                                              |
| `testObjectIdInRange` | yes      | `(objectId: number, hitmapProps: HitmapProp<T>) => boolean` |         | test if the `objectId` returned by the hitmap event is in the marker's id range when rendering instanced objects |
