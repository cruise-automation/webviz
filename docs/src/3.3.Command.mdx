...
| `mapDrawPropToHitmapProp`  | yes       | `(T, number) => ?T`                                               |         | map the marker object to an object that should be used instead when rendering to the hitmap for detecting mouse events. See Mouse Events for more |
