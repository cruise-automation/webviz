...
| `getChildrenForHitmap`     | `AssignNextIdsFn` |         | Maps the `children` to props to be drawn on the hitmap. Optional, but required for any mouse interactions. Provided by default by all included Commands. See more below. |
...
<Worldview>
  <Points
    ...
    getChildrenForHitmap={(props, assignNextIds) => {
      const hitmapProps = nonInstancedGetChildrenForHitmap(props, assignNextIds);
