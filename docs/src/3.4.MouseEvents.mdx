...
Some commands may want to do something special when the user interacts with them. For example, a Point instance may want to allow clicking close to a point and not directly on a point to still trigger a click event. `mapDrawObjectToHitmapObject` allows this by taking in the object to be rendered and returning a new object that will be rendered into the hitmap only.
...
<Worldview>
  <Points
    ...
    mapDrawObjectToHitmapObject={(prop, index) => {
