...
Each Command is passed the `reglCommand` function and `children`. Worldview compiles the `reglCommand` once by calling it with the `regl` object as an argument, and calls the resulting command each frame with `children`. `getHitmap` takes in those `children` and returns new version of them that will be rendered into the hitmap.
...
Example using composition of the default `getHitmap` functions:

```js
<Worldview>
  <Points
    onClick={(evt, { objects }) => console.log(`You clicked on ${objects[0].object.id}`)}
    getHitmap={(props, assignNextIds) => {
...
Example using a custom `getHitmap` function:
