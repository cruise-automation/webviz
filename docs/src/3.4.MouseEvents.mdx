...
**Custom**
You can also customize hitmap id mapping through `getHitmapProps` and `getObjectFromHitmapId` props. Both the two props must be present as providing only one may cause incorrect mapping.
...
_Commands that currently support instanced rendering: Cubes, Points, Spheres, Triangles, Cones, Cylinders_
