...
- `interactedObjectId`: the `id` that you specified in the regl commands (`undefined` if it's not specified).

Sample usage:

```js
<Worldview onClick={(evt, { interactedObjectId }) => console.log("You clicked on", interactedObjectId)}>
  {/* Command children */}
</Worldview>
```
...
- `ray`: the raw raycasting information including `dir` (direction), `origin`, and `point` (all in `Vec3` format).
- `interactedObject`: the object that you interacted with.
