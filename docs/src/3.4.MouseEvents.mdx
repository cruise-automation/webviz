...
- `objectId`: the `id` that you specified in the regl commands (`undefined` if it's not specified).
...
```js
<Worldview>
  <Cubes onClick={(evt, { object }) => console.log("You clicked on", object)}>{cubes}</Cubes>
</Worldview>
```
