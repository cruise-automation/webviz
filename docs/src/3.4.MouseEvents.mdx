...
| `getHitmap`     | `CommandBoundAssignNextIds` |                                                                            | Maps the `drawProps` to props to be drawn on the hitmap. Optional, but required for any mouse interactions. Provided by default by all included Commands. See more below. |
...
### getHitmap and mapping for interactive objects
...
The Worldview library allows a lot of control over this interaction, but most Commands can use one of two default `getHitmap` functions.

| Name | Description | Usage |
| ---- | ----------- | ----- |
| `nonInstancedGetHitmap` | A default `getHitmap` function for non-instanced commands. | `<Command getHitmap={nonInstancedGetHitmap} ... />` |
| `createInstancedGetHitmap` | A factory function for instanced commands. It takes the number of points per instance: for example, a triangle has 3 points, whereas a sphere has 1 point (just the center). | `<Command getHitmap={createInstancedGetHitmap({ pointCountPerInstance: 1 })} ... />`

##### Turning off interactivity for a layer

All built in commands allow overriding the `getHitmap` function, and we recommend that you also allow users to override it. If a user passes in `null` or `undefined` for `getHitmap` in a command, any objects drawn by that command instance will not be visible for interaction purposes. Disabling interaction in this way may help improve performance.

##### Customizing the getHitmap function

Each Command is passed the `reglCommand` function `drawProps` for rendering into the scene using the `reglCommand` function. `getHitmap` takes in those `drawProps` and returns new version of them that will be rendered into the hitmap.

Some important things to remember:
 * The first argument of `getHitmap` is the `drawProps`.
 * The second argument is a function that helps us assign IDs to each object.
```js
/*
 ...
 */
type CommandBoundAssignNextIds = (
  { type: "single", callbackObject: BaseShape } | { type: "instanced", callbackObject: BaseShape, count: number }
) => Array<HitmapId>;
```
 * Each unique object that you want to event on should get a unique ID. If you want to map parts of a single object to multiple IDs (instancing), use the `type: "instanced"` call to `assignNextIds`.
...
Example using composition of the default `mapHitmap` functions:

```js
<Worldview>
  <Points
    onClick={(evt, { objects }) => console.log(`You clicked on ${objects[0].object.id}`)}
    mapHitmap={(props, assignNextIds) => {
...
Example using a custom `mapHitmap` function:
...
<Worldview>
  <Points
    onClick={(evt, { objects }) => console.log(`You clicked on ${objects[0].object.id}`)}
    mapHitmap={(props, assignNextIds) => {
      // This is a simplified version of nonInstancedGetHitmap, for explanation purposes.
      return propsArray.map((prop) => {
        ...
        // We turn the hitmapId into a color and use it for the points.
