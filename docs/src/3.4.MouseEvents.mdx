...
| `getHitmap`     | `AssignNextIdsFn` |                                                                            | Maps the `drawProps` to props to be drawn on the hitmap. Optional, but required for any mouse interactions. Provided by default by all included Commands. See more below. |
...
Each Command is passed the `reglCommand` function and `drawProps`. Worldview calls compiled version of the `reglCommand` function with `drawProps`. `getHitmap` takes in those `drawProps` and returns new version of them that will be rendered into the hitmap.
