...
export function splitTopicPathOnTopicName(path: string): ?{ topicName: string, trailingPath: string } {
  const rosPath = parseRosPath(path);
  return rosPath ? { topicName: rosPath.topicName, trailingPath: path.slice(rosPath.topicName.length) } : undefined;
}
