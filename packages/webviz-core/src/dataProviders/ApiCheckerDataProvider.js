  ...
  async initialize(extensionPoint: ExtensionPoint): Promise<InitializationResult> {
    ...
    for (const topic of initializationResult.topics) {
      if (initializationResult.messageDefinitions.type === "raw") {
        ...
        if (!initializationResult.messageDefinitions.datatypes[topic.datatypeId]) {
          this._warn(
            `Topic "${topic.name}" datatype "${topic.datatypeName}" (${topic.datatypeId}) not present in datatypes`
          );
        }
      }
    }
