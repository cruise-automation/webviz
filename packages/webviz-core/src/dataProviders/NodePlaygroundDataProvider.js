...
export const getMessageDefinitionForDatatype = (datatypes: RosDatatypes, datatypeId: string): RosMsgDefinition[] => {
  ...
  const depthFirstSearch = (dfsDatatype) => {
    ...
    const { name, fields } = datatypes[dfsDatatype];
    fields.forEach(({ type }) => {
      if (isComplex(type)) {
        depthFirstSearch(type);
      }
    });
    ret.push({ name, definitions: fields });
    complexDatatypesUsed.add(dfsDatatype);
  };
  depthFirstSearch(datatypeId);
  return ret.reverse();
};
        ...
        nodeMessages.forEach(({ message, topic, receiveTime }) => {
          const bobject = isBobject(message)
            ...
            : {
                topic,
                receiveTime,
                message: wrapJsObject(this._allDatatypes, output.datatypeId, message),
              };
  ...
  _getMessageDefinitions(): ParsedMessageDefinitions {
    ...
    objectValues(this._nodeRegistrationsByTopic).forEach(({ registration }) => {
      ...
      parsedMessageDefinitionsByTopic[registration.output.name] = getMessageDefinitionForDatatype(
        datatypes,
        registration.output.datatypeId
      );
    });
    return {
      type: "parsed",
