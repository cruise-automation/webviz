...
import ApiCheckerDataProvider, {instrumentTreeWithApiCheckerDataProvider,} from 'webviz-core/src/dataProviders/ApiCheckerDataProvider';
...
import MeasureDataProvider, {instrumentTreeWithMeasureDataProvider,} from 'webviz-core/src/dataProviders/MeasureDataProvider';
...
import RewritePbBinaryDataProvider from 'webviz-core/src/dataProviders/RewritePbBinaryDataProvider';
...
const getDataProviderBase = createGetDataProvider({
  ...
  RewritePbBinaryDataProvider,
  WorkerDataProvider,
  ...getGlobalHooks().getAdditionalDataProviders(),
});

export function rootGetDataProvider(tree: DataProviderDescriptor):
    DataProvider {
  if (new URLSearchParams(location.search)
          .has(MEASURE_DATA_PROVIDERS_QUERY_KEY)) {
