...
import { type ComponentReglClickInfo } from "regl-worldview";
...
export default class MeasuringTool extends React.Component<Props> implements MarkerProvider {
  ...
  _canvasMouseDownHandler = (e: MouseEvent, clickInfo: ComponentReglClickInfo) => {
    this.mouseDownCoords = [e.clientX, e.clientY];
  };

  _canvasMouseUpHandler = (e: MouseEvent, clickInfo: ComponentReglClickInfo) => {
  ...
  _canvasMouseMoveHandler = (e: MouseEvent, clickInfo: ComponentReglClickInfo) => {
    const { measureState, measurePoints, onMeasureInfoChange } = this.props;
    switch (measureState) {
      case "place-start":
        onMeasureInfoChange({
  ...
  get canvasMouseMove(): ?(MouseEvent, ComponentReglClickInfo) => void {
  ...
  get canvasMouseUp(): ?(MouseEvent, ComponentReglClickInfo) => void {
    if (!this.measureActive) {
      return null;
    }
  ...
  get canvasMouseDown(): ?(MouseEvent, ComponentReglClickInfo) => void {
    if (!this.measureActive) {
      return null;
    }
