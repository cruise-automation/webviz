...
import {RADAR_POINT_CLOUD, SENSOR_MSGS$POINT_CLOUD_2, WRAPPED_POINT_CLOUD,} from 'webviz-core/src/util/globalConstants';

export type StructuralDatatypes = {
  [topicName: string]: 'radarPointCloud'
};
...
function isRadarPointCloud(typeName, datatypes) {
  ...
  return ['range', 'azimuth_angle_0', 'elevation_angle', 'radial_vel'].every(
      (fieldName) => {
...
function isWrappedPointCloud(typeName, datatypes) {
  ...
  return cloudField && !cloudField.isArray &&
      cloudField.type === SENSOR_MSGS$POINT_CLOUD_2;
}
