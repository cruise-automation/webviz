...
describe("typegen", () => {
  describe("generateTypeDefs", () => {
    ...
    describe("internal references", () => {
      ...
      it("handles ambiguous datatype names", () => {
        const declarations = generateTypeDefs({
          "opaque_id/datatype0": {
            name: "nonstd_msgs/ParentMsg",
            fields: [
              { type: "opaque_id/datatype1", name: "x", isArray: false, isComplex: true },
              { type: "opaque_id/datatype2", name: "y", isArray: false, isComplex: true },
            ],
          },
          "opaque_id/datatype1": { name: "ambiguous_msgs/Msg", fields: [] },
          "opaque_id/datatype2": { name: "ambiguous_msgs/Msg", fields: [] },
        });
        expect(formatTypeDef(declarations)).toEqual({
          "opaque_id/datatype0": `export interface nonstd_msgs__ParentMsg { x: ambiguous_msgs__Msg; y: ambiguous_msgs__Msg_1; }`,
          "opaque_id/datatype1": `export interface ambiguous_msgs__Msg { }`,
          "opaque_id/datatype2": `export interface ambiguous_msgs__Msg_1 { }`,
        });
      });
    });
  ...
  describe("generateRosLib", () => {
    ...
    it("works with ambiguous datatypes", () => {
      const rosLib = generateRosLib({
        topics: [
          {
            name: "/my_topic_0",
            datatypeName: "non_msgs/Ambiguous",
            datatypeId: "id0",
          },
          {
            name: "/my_topic_1",
            datatypeName: "nonstd_msgs/Ambiguous",
            datatypeId: "id1",
          },
        ],
        datatypes: {
          id0: { name: "nonstd_msgs/Ambiguous", fields: [] },
          id1: { name: "nonstd_msgs/Ambiguous", fields: [] },
        },
      });
      const { diagnostics } = compile({ ...baseNodeData, sourceCode: rosLib });
      expect(diagnostics).toEqual([]);
