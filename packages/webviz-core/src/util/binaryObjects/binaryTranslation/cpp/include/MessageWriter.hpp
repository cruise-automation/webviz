...
struct DataWriter {
    ...
    inline bool write(const uint8_t* src, size_t size) {
        if (next + size > end) {
            return false;
        }
    ...
    inline bool writeOffsets(uint32_t count, uint32_t begin) {
...
class DataReader {
public:
    DataReader(const uint8_t* data, size_t size) : _data(data), _size(size) {}
    ~DataReader() = default;

    inline bool readLength(uint32_t* length) {
    ...
    inline bool read(size_t size, DataWriter* dst) {
        if (_read + size > _size) {
            return false;
        }
...
public:
    ...
    bool reserve(Definition* definition, size_t messageCount, size_t totalBytes);
    ...
    int32_t write(Definition* definition, uintptr_t data, size_t size);

#if defined(__EMSCRIPTEN__)
    emscripten::val getDataBufferJS();
    emscripten::val getStringBufferJS();
#endif

private:
    DataWriter allocate(Buffer* data, size_t size);
    ...
    bool dispatchCommands(const Definition::CommandBuffer& cmds, DataReader* src, DataWriter* dst);
