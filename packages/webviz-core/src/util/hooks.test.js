...
describe("useDebouncedValue", () => {
  function useDebounceExample(initialVal) {
    const [value, setValue] = useState(initialVal);
    const debouncedValue = useDebouncedValue(value, 500);
    return { debouncedValue, setValue };
  }

  it("returns the debounced value for primitive values", async () => {
    const { result, waitForNextUpdate } = renderHook(() => useDebounceExample(1));
  ...
  it("returns the debounced value for objects", async () => {
    const initialVal = { a: 1 };
    const newVal = { b: 2 };
    const { result, waitForNextUpdate } = renderHook(() => useDebounceExample(initialVal));
    expect(result.current.debouncedValue).toBe(initialVal);
    act(() => {
    ...
    expect(result.current.debouncedValue).toBe(newVal);
  });
});
